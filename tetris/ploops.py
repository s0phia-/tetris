...
def p_loop(p, seed, plot_individual=False, return_action_count=False):
    ...
    if p.notify:
        notify_ending(str(seed) + " successfully ended!")
...
def cbmpi_learn_loop(p, seed, D):
    ...
    start = time.time()
    tested_weights_ix, tested_value_weights_ix = environment.learn_cbmpi(num_iter=p.num_tests)
    end = time.time()
    total_time = end - start
    return [tested_weights_ix, tested_value_weights_ix]
