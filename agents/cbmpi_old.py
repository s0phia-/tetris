...
class CmaesClassifier:
    def __init__(self, cmaes_var, num_features, min_iterations, seed=0):
        self.n = int(num_features * 15)  # CMA-ES parameter ('popsize' in cma)
        self.min_iterations = min_iterations

        self.policy_weights = np.random.normal(loc=0, scale=cmaes_var, size=num_features)
        self.cmaes = cma.CMAEvolutionStrategy(
            np.random.normal(loc=0, scale=1, size=num_features),
            cmaes_var,
            inopts={'verb_disp': 0,
                    'verb_filenameprefix': "output/cmaesout" + str(seed),
                    'popsize': self.n})

    def fit(self, did_rollout, state_action_features, num_available_actions, state_action_values):
...
class CbmpiOld:
    def __init__(self, m, N, M, B, D,
        ...
        # else:
        #     self.policy_approximator = cma.CMAEvolutionStrategy(
    ...
    def learn(self):
        ...
        for ix, rollout_state in enumerate(self.D_k):
            ...
            self.state_values[ix] = value_roll_out(rollout_state, self.m, self.gamma, self.generative_model,
                                                   self.policy_weights, self.value_weights, self.num_features)
            actions_value_estimates, state_action_features = \
                action_value_roll_out(rollout_state, self.m, self.gamma, self.generative_model,
                                      self.policy_weights, self.value_weights, self.num_features)
        ...
        if self.discrete_choice:
            ...
            self.policy_weights = self.classifier.fit(self.did_rollout, self.state_action_features, self.num_available_actions, self.state_action_values)
